              ...
              >
                ...
                {signals[timeframe] && (
                  <div className="space-y-4">
                    ...
                    <div className={`p-4 rounded-lg border-2 ${getSignalBgClass(signals[timeframe]!.direction)}`}>
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="text-xl font-bold text-white">
                            {signals[timeframe]!.direction} Signal
                          </h3>
                          <p className="text-gray-300 text-sm">
                            {timeframe} timeframe • {signals[timeframe]!.confidence}% confidence
                          </p>
                        </div>
                        <Badge 
                          variant="outline" 
                          className={`text-xs font-bold ${
                            signals[timeframe]!.confidence >= 70 ? 'text-green-400 border-green-500' :
                            signals[timeframe]!.confidence >= 50 ? 'text-yellow-400 border-yellow-500' :
                            'text-red-400 border-red-500'
                          }`}
                        >
                          {signals[timeframe]!.confidence >= 70 ? 'HIGH' :
                           signals[timeframe]!.confidence >= 50 ? 'MED' : 'LOW'}
                        </Badge>
                      </div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                        ...
                        <div className="text-center">
                          ...
                          <div className="text-sm font-bold text-purple-400">
                            {typeof signals[timeframe]!.riskRewardRatio === 'number' ? 
                              signals[timeframe]!.riskRewardRatio.toFixed(1) : '1.5'}
                          </div>
                        </div>
                      </div>

                      {/* Recommendation Summary */}
                      {recommendation && recommendation.timeframe === timeframe && (
